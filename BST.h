...
void inOrdersite( TreeNodePtr treePtr,int deep)
{ 
   int i;
   // if tree is not empty, then traverse
   if ( treePtr != NULL ) {        
      deep++;          
      inOrdersite( treePtr->rightPtr ,deep); //Recursion to the right
      for(int i=1; i<deep;i++)printf("   ");
      printf("%3d\n",treePtr->data) ;  //print the value 
      inOrdersite( treePtr->leftPtr ,deep); //Recursion to the left
   } // end if
} // end 
